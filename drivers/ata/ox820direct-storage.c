...
int alloc_direct_sata_context(
	...
	direct_access_context_t **context)
{
	static const int PHYS_DEV_MAJOR = 8;	// sda
	static const int PHYS_DEV_MINOR = 2;	// sda2
	static const int LVM_OFFSET = 384;	// 192KB of 512 byte sectors
	...
	bd = bdget(MKDEV(PHYS_DEV_MAJOR, PHYS_DEV_MINOR));
	if (!bd) {
		retval = -ENODEV;
		goto out;
	}
    ...
    single_context->start     = partition->start_sect + LVM_OFFSET;
